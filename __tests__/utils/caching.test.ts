...
describe('Response Caching Headers', () => {
  ...
  describe('Cache Control Headers', () => {
    it('should set appropriate cache headers for cacheable responses', async () => {
      const response = await request(app).get('/cached-endpoint').expect(200);
    ...
    it('should set no-cache headers for non-cacheable responses', async () => {
      const response = await request(app).get('/no-cache-endpoint').expect(200);
    ...
    it('should handle conditional requests with ETags', async () => {
      ...
      const firstResponse = await request(app).get('/conditional-cache').expect(200);

      const etag = firstResponse.headers['etag'];
      expect(etag).toBe('"789012"');

      // Second request with If-None-Match
      await request(app).get('/conditional-cache').set('If-None-Match', etag!).expect(304);
    });
  });

...
export class AddressPreprocessor {
      ...
      if (corrections) {
        ...
        if (corrections.hasOwnProperty(normalizedCity)) {
          ...
          if (correctedCity === null) {
            ...
            if (zipCode && ZIP_TO_CITY[zipCode]) {
              logger.warn(
                `Invalid city "${city}" corrected to "${ZIP_TO_CITY[zipCode].city}" using ZIP ${zipCode}`
              );
              return ZIP_TO_CITY[zipCode].city;
            }
   ...
   */
  shouldRetryWithoutCity(error: unknown, hasCity: boolean, hasZipCode: boolean): boolean {
    // Only retry if we have both city and ZIP, and got a 400 error
    return (
      (error as { response?: { status: number } }).response?.status === 400 && hasCity && hasZipCode
    );
  }

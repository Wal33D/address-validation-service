...
function formatOutput(data: unknown, format: string = 'json'): string {
  if (format === 'json') {
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      ...
      const response = await axios.post(
        `${options.host as string}/validate-location`,
        requestBody,
        {
      ...
      if (response.status === 200) {
        console.log('✅ Success!\n');
        console.log(formatOutput(response.data, options.format as string | undefined));
      } else {
        console.error(`❌ Error (${response.status}):\n`);
        console.error(formatOutput(response.data, options.format as string | undefined));
        process.exit(1);
      }
    } catch (error) {
...
program
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      const response = await axios.get(`${options.host as string}/health`);
...
program
  ...
  .action(async (options: Record<string, unknown>) => {
    try {
      const response = await axios.get(`${options.host as string}/cache/stats`);

...
describe('AddressPreprocessor', () => {
    describe('preprocessStreetAddress', () => {
        ...
        it('should handle multiple abbreviations in one address', () => {
            expect(addressPreprocessor.preprocessStreetAddress('123 N Main St'))
                .toBe('123 N. Main St.');
            expect(addressPreprocessor.preprocessStreetAddress('456 SE River Rd'))
                .toBe('456 SE. River Rd.');
        });
        ...
        it('should handle the specific error case from analysis', () => {
            expect(addressPreprocessor.preprocessStreetAddress('6470 S Stony Road'))
                .toBe('6470 S. Stony Road');
        });
    });

    describe('validateCity', () => {
        it('should correct known Michigan city issues', () => {
            expect(addressPreprocessor.validateCity('St Joseph', 'MI', undefined))
                .toBe('Saint Joseph');
            expect(addressPreprocessor.validateCity('St Clair', 'MI', undefined))
                .toBe('Saint Clair');
        });

        it('should return null for invalid city with no ZIP', () => {
            expect(addressPreprocessor.validateCity('McBride', 'MI', undefined))
                .toBeUndefined();
        });

        it('should use ZIP lookup for invalid city', () => {
            expect(addressPreprocessor.validateCity('McBride', 'MI', '48852'))
                .toBe('Mount Pleasant');
        });

        it('should use ZIP lookup when no city provided', () => {
            expect(addressPreprocessor.validateCity(undefined, 'MI', '48852'))
                .toBe('Mount Pleasant');
        });
        ...
        it('should return original city if no corrections needed', () => {
            expect(addressPreprocessor.validateCity('Detroit', 'MI', undefined))
                .toBe('Detroit');
        });
    });
    ...
    describe('dynamic mappings', () => {
        ...
        it('should allow adding city corrections', () => {
            ...
            expect(addressPreprocessor.validateCity('St Clairsville', 'OH', undefined))
                .toBe('Saint Clairsville');
        });
    });
});